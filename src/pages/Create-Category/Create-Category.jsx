...
const updateFormik = useFormik({
  ...
  const updateCategory=async(cat)=>{
    try {
      const {data}=await axios.put(`https://ecomnode.onrender.com/api/v1/category/update-category/${selected._id}`,cat)
  if(data.success){
...
{loading?<h3>Loading....</h3>:category?.categories?.map((cat,i)=>(
  <>
   <Table key={cat._id} striped bordered hover variant="dark" className='text-center mx-sm-auto col-sm-12'>  
      <thead>
        <tr>
      ...
      <tbody>
        <tr className='text-center'>
          ...
          <td className='w-50'> 
          <button className='btn btn-danger mx-2' onClick={()=>handleDelete(cat._id)}>Delete</button>
    ...
    <Modal
              ...
              open={visible}
            >
          <form className="my-3 w-75" id='updateForm' onSubmit={updateFormik.handleSubmit}>
