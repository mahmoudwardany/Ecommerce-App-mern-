...
const dispatch=useDispatch()
    ...
    const fetchProductDetails=async()=>{
       try {
         const {data}=await axios.get(`https://ecomnode.onrender.com/api/v1/product/${params.slug}`)
  ...
  return (
    <div className='row container mt-4' >
     <div className='col-md-6 text-center mx-auto'>
        <img src={`http://localhost:5000/api/v1/product/get-photo/${product?._id}`}
         alt={product?.name}
         height="300"
         width={"350px"}
         />
     ...
     <button className="btn btn-secondary ms-1"onClick={()=>dispatch(addtoCart(product))} >ADD TO CART</button>
     <button className="btn btn-success ms-1" onClick={()=>  nav(`/`)}>Back To Shopping</button>
     </div>
     <div className="row">
 ...
 <div className=' container  col-md-4 mb-3' key={p._id}>
                ...
                <div key={p._id} className="card mx-sm-auto text-sm-center" style={{ width: "18rem",height:"550px" }}>
                  <img
